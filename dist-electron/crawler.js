...
class Crawler {
    constructor(window) {
        ...
        this.sessionDir = path.join(electron_1.app.getPath('userData'), 'browser-sessions');
        if (!fs.existsSync(this.sessionDir)) {
            fs.mkdirSync(this.sessionDir, { recursive: true });
        }
    }
    async start(url, useSession = false, options) {
        try {
    ...
    // 현재 세션 저장
    async saveCurrentSession(hostname) {
        if (!this.context)
            return;
    ...
    async extractData(page, url, options) {
    ...
    // 도메인 필터 적용
    applyDomainFilter(link, domainSettings) {
        ...
        try {
            ...
            const matchesAnyPattern = filter.patterns.some(pattern => this.matchPattern(link, pattern));
            // 화이트리스트: 패턴 매칭되면 허용
